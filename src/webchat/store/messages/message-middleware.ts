...
const NO_TRANSLATE_PREFIX = 'NoTranslate:';
...
export const createMessageMiddleware = (client: SocketClient): Middleware<{}, StoreState> => store => next => (action: SendMessageAction | ReceiveMessageAction | TriggerEngagementMessageAction | TriggerInjectionMessageAction) => {
  switch (action.type) {
    case 'SEND_MESSAGE': {
      ...
      if (!settings.disableTextInputSanitization) {
        text = sanitizeHTML(text || '');
      }

      if (settings.disableHtmlInput) {
        text = new DOMParser()
      ...
      if (!settings.showQuickReplyPayload && typeof options.label === 'string') {
        displayMessage.text = options.label;
      } else if (text) { // Display the message payload instead of Quick Reply button title
        displayMessage.text = text.startsWith(NO_TRANSLATE_PREFIX) ? // Show the text without NoTranslate:
          text.substring(NO_TRANSLATE_PREFIX.length) :
          text;
      }
