...
const InputForm = styled.form(({ theme }) => ({
  ...
  // "&[data-active=true]": {
  //   borderBottomColor: theme.primaryColor,   .. to remove the red-line occuring on the bot
  // },
}));
...
> {
  ...
  handleMenuButton = () => {
    const mode = this.state.mode === "menu" ? "text" : "menu";

    this.setState({ mode }, () => {
      this.props.onEmitAnalytics(
        mode === "menu" ? "open-persistent-menu" : "close-persistent-menu"
      );
    });
  };
  ...
  handleMenuKeyDown = (event) => {
    ...
    switch (key) {
      case "ArrowUp":
        newFocusTarget =
          target.previousElementSibling || this.menuRef.current?.lastChild;
        break;
      case "ArrowDown":
        newFocusTarget =
          target.nextElementSibling || this.menuRef.current?.firstChild;
        break;
      case "Home":
   ...
   */
  handleInputKeyDown: React.KeyboardEventHandler<HTMLTextAreaElement> = (
    event
  ) => {
  ...
  render() {
    ...
    return (
      ...
      >
        ...
        {mode === "text" && (
          <>
            {this.props.config.settings.bayer.displayLanguageSelectMenu ? (
              <LanguageSelect
            ...
            {this.props.config.settings.bayer.displayInputField && (
              <InputField>
                {!disableInputAutogrow ? (
                  <Translate>
                    {({ translate }) => (
                      <TextArea
                        ...
                        placeholder={translate("UI_INPUT_PLACEHOLDER")}
        ...
        {mode === "menu" && (
          ...
          >
            {title && (
              <PersistentMenuTitle
                className="webchat-input-persistent-menu-title"
